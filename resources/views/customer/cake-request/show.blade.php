...
@if(session('info'))
<div class="success-toast" style="background:#EBF3FE; border-color:#BEDAF5;">
    <div class="toast-icon">🏪</div>
    <div>
        <div class="toast-title" style="color:#1A3A6B;">Pickup Order</div>
        <div class="toast-sub" style="color:#2E5FA3;">{{ session('info') }}</div>
    </div>
</div>
@endif
...
if ($cakeRequest->is_rush) {
        $steps = [
            ...
            ['key'=>'ACCEPTED',              'label'=>'⚡ Matched','icon'=>'⚡'],
        ...
        $order = ['OPEN','ACCEPTED','WAITING_FOR_PAYMENT','IN_PROGRESS','WAITING_FINAL_PAYMENT','COMPLETED'];
    } else {
        $steps = [
            ['key'=>'OPEN',                  'label'=>'Submitted', 'icon'=>'📋'],
        ...
        $order = ['OPEN','BIDDING','ACCEPTED','WAITING_FOR_PAYMENT','IN_PROGRESS','WAITING_FINAL_PAYMENT','COMPLETED'];
    }
...
$acceptedBid        = null;
    ...
    if (in_array($cakeRequest->status, ['ACCEPTED','WAITING_FOR_PAYMENT','WAITING_FINAL_PAYMENT','IN_PROGRESS','COMPLETED'])) {
        ...
        if ($acceptedBid) {
            $downpaymentAmount = round($acceptedBid->amount * 0.5, 2);
...
@php
    $nextInfo = [
        ...
        'BIDDING'                => ['icon'=>'🔔','color'=>'yellow','title'=>'You have bids!',                  'desc'=>'Review each baker\'s offer below — check their price, estimated days, and message, then accept the best one.'],
...
@endif
                    ...
                    <div style="flex:1; min-width:0;">
                        <div class="bid-name">{{ $bid->baker->first_name }} {{ $bid->baker->last_name }}</div>
                        <div class="bid-meta">
...
<button type="button" class="btn-accept"
    ...
    ✓ View Bid ( ₱{{ number_format($bid->amount, 0) }} )
</button>
                </div>
            </div>
            @endforeach
...
@endif
    ...
    <div>
        <div class="bids-summary-card" style="margin-bottom:1.5rem;">
            ...
            <div class="bss-row"><span class="bss-key">Delivery</span><span class="bss-val" style="color:var(--caramel); font-weight:700;">{{ $cakeRequest->delivery_date->format('M d, Y') }}</span></div>
 ...
 {{-- RIGHT SIDEBAR (OPEN state) --}}
    <div>
        ...
        <div class="card">
            ...
            <div class="info-row"><span class="i-key">Delivery Date</span><span class="i-val" style="font-weight:700; color:var(--caramel);">{{ $cakeRequest->delivery_date->format('M d, Y') }}</span></div>
     ...
     {{-- Cake Design + Order Details (combined) --}}
        <div class="card">
            ...
            <div style="border-top:1px solid var(--border);">
                ...
                <div class="info-row"><span class="i-key">Budget</span><span class="i-val">₱{{ number_format($cakeRequest->budget_min,0) }} — ₱{{ number_format($cakeRequest->budget_max,0) }}</span></div>
                <div class="info-row"><span class="i-key">Date</span><span class="i-val" style="font-weight:700; color:var(--caramel);">{{ $cakeRequest->delivery_date->format('M d, Y') }}</span></div>
            ...
            <div class="psc-header">
                <div class="psc-header-left">
                    ...
                    <div>
                        ...
                        <div class="psc-sub">₱{{ number_format($acceptedBid->amount, 2) }} · 50/50 split</div>
                    </div>
                </div>
            </div>
            <div class="psc-split">
...
@endif
@endsection
{{-- ── RUSH ORDER AUTO-MATCH MODAL ── --}}
@if($cakeRequest->is_rush && $cakeRequest->status === 'ACCEPTED' && session('show_rush_modal'))
@php
    $rushBakerOrder = $cakeRequest->bakerOrder;
    $rushBakerUser  = $rushBakerOrder?->baker;
    $rushBakerProf  = \App\Models\Baker::where('user_id', $rushBakerOrder?->baker_id)->first();
@endphp
<div class="modal-backdrop open" id="rushMatchModal" role="dialog" aria-modal="true" style="z-index:10001;">
    <div class="modal-box" style="max-width:680px; width:95vw; max-height:92vh; overflow:hidden; display:flex; flex-direction:column; border-radius:24px;">

        {{-- Header --}}
        <div class="modal-header" style="background:linear-gradient(135deg,#1A0A00,#5C3010); flex-shrink:0; padding:1.5rem 2rem 1.25rem;">
            <span class="modal-header-icon">⚡</span>
            <div class="modal-header-title">Rush Baker Matched!</div>
            <div class="modal-header-sub">The nearest available baker has been assigned to your order.</div>
        </div>
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr; flex:1; min-height:0; overflow:hidden;">
            ...
            <div style="padding:1.25rem; border-right:1px solid var(--border,#EAE0D0); display:flex; flex-direction:column; gap:0.75rem; overflow-y:auto;">
                ...
                {{-- Baker row --}}
                <div style="display:flex; align-items:center; gap:0.75rem; background:#FDF6F0; border:1px solid #F0E0C8; border-radius:12px; padding:0.75rem 1rem;">
                    <div style="width:38px;height:38px;border-radius:50%;background:linear-gradient(135deg,#C07840,#E8A96A);color:white;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:1rem;flex-shrink:0;overflow:hidden;">
                        @if($rushBakerUser?->profile_photo)
                            <img src="{{ asset('storage/'.$rushBakerUser->profile_photo) }}" style="width:100%;height:100%;object-fit:cover;">
                        @else
                            {{ strtoupper(substr($rushBakerUser?->first_name ?? '?', 0, 1)) }}
                        @endif
                    </div>
                    <div style="flex:1; min-width:0;">
                        <div style="font-weight:700; font-size:0.88rem; color:var(--brown-deep,#3B1F0F);">
                            {{ $rushBakerUser?->first_name }} {{ $rushBakerUser?->last_name }}
                        </div>
                        <div style="font-size:0.68rem; color:var(--text-muted,#9C8070); margin-top:0.1rem;">
                            ⚡ Auto-matched · 1-day estimate
                        </div>
                    </div>
                    <div style="text-align:right; flex-shrink:0;">
                        <div style="font-size:0.82rem; font-weight:700; color:var(--caramel,#C8894A);">
                            @if($rushBakerProf?->rating) ★ {{ number_format($rushBakerProf->rating, 1) }} @else New Baker @endif
                        </div>
                        <div style="font-size:0.62rem; color:var(--text-muted,#9C8070);">
                            {{ $rushBakerProf?->total_reviews ?? 0 }} reviews
                        </div>
                    </div>
                </div>
                ...
                <div>
                    @php
                        $bLat = (float)($rushBakerProf?->latitude ?? 0);
                        $bLng = (float)($rushBakerProf?->longitude ?? 0);
                        $cLat = (float)($cakeRequest->delivery_lat ?? 0);
                        $cLng = (float)($cakeRequest->delivery_lng ?? 0);
                        $distKm = null;
                        if ($bLat && $bLng && $cLat && $cLng) {
                            $R = 6371;
                            $dLat = deg2rad($bLat - $cLat);
                            $dLng = deg2rad($bLng - $cLng);
                            $a = sin($dLat/2)**2 + cos(deg2rad($cLat))*cos(deg2rad($bLat))*sin($dLng/2)**2;
                            $distKm = round($R * 2 * atan2(sqrt($a), sqrt(1-$a)), 1);
                        }
                    @endphp
                    <div style="display:flex; align-items:center; gap:0.5rem; margin-bottom:0.35rem; flex-wrap:wrap;">
                        @if($distKm !== null)
                        <div style="font-size:0.73rem; font-weight:700; color:var(--caramel,#C8894A); background:#FEF3E8; padding:0.2rem 0.65rem; border-radius:20px; border:1px solid rgba(200,137,74,0.25);">
                            📍 {{ $distKm < 1 ? round($distKm * 1000).' m' : $distKm.' km' }} away
                        </div>
                        @endif
                        <div style="display:inline-flex; align-items:center; gap:0.3rem; font-size:0.7rem; font-weight:600; padding:0.2rem 0.65rem; border-radius:20px; background:#FEF9E8; color:#8A5010; border:1px solid #F0D090;">⚡ Rush · 1-day</div>
                    </div>
                    @if($distKm !== null)
                    <div style="background:{{ $distKm <= 5 ? '#FEF9E8' : '#EBF3FE' }}; border:1.5px solid {{ $distKm <= 5 ? '#F0D090' : '#BFDBFE' }}; border-radius:8px; padding:0.55rem 0.75rem; font-size:0.73rem; line-height:1.5; color:{{ $distKm <= 5 ? '#8A5010' : '#1A3A6B' }}; margin-bottom:0.4rem;">
                        @if($distKm <= 5)
                            💡 <strong>The Baker is nearby — Pickup is a great option!</strong> Save on delivery and pay the final 50% in cash on pickup.
                        @else
                            <strong>The Baker is {{ $distKm }} km away — Delivery is recommended.</strong> The baker will bring your cake to your address.
                        @endif
                    </div>
                    @endif
                    <div style="font-size:0.73rem; color:var(--text-muted,#9C8070); line-height:1.4;">
                        {{ $rushBakerProf?->address ?? 'Baker address not listed' }}
                    </div>
                </div>

                {{-- Map --}}
                <div style="flex:1; min-height:180px; position:relative;">
                    <div id="rushMatchMap" style="width:100%; height:100%; min-height:180px; border-radius:10px; border:1px solid var(--border,#EAE0D0); overflow:hidden;"></div>
                    <div style="position:absolute; bottom:6px; left:50%; transform:translateX(-50%); background:rgba(255,255,255,0.92); border-radius:20px; padding:3px 10px; font-size:0.62rem; color:var(--text-muted); display:flex; align-items:center; gap:6px; white-space:nowrap; box-shadow:0 1px 4px rgba(0,0,0,0.12);">
                        <span>🔵 You</span><span>·</span><span>🟠 Baker</span>
                    </div>
                </div>
                ...
                {{-- Specialties --}}
                @if($rushBakerProf?->specialties)
                <div style="display:flex; flex-wrap:wrap; gap:0.3rem;">
                    @foreach(array_slice((array)$rushBakerProf->specialties, 0, 5) as $spec)
                    <span style="padding:0.15rem 0.55rem; background:var(--cream,#FDF6F0); border:1px solid var(--border); border-radius:6px; font-size:0.62rem; color:var(--text-muted); font-weight:600;">{{ trim($spec) }}</span>
                    @endforeach
                </div>
                @endif
            </div>
            ...
            <div style="padding:1.25rem; display:flex; flex-direction:column; gap:0.75rem; overflow-y:auto;">
                ...
                {{-- Fulfillment --}}
                <div>
                    <div style="font-size:0.62rem; text-transform:uppercase; letter-spacing:0.1em; color:var(--text-muted,#9C8070); font-weight:600; margin-bottom:0.5rem;">How will you receive your cake?</div>
                    <div style="display:grid; grid-template-columns:1fr 1fr; gap:0.5rem;" id="rushMatchFulfillmentToggle">
                        <label id="rmf-delivery" style="border:2px solid var(--caramel,#C8894A); border-radius:10px; padding:0.6rem 0.7rem; cursor:pointer; background:#FEF3E8; position:relative; transition:all 0.2s;">
                            <input type="radio" name="rush_match_fulfillment" value="delivery" checked style="position:absolute;opacity:0;width:0;height:0;">
                            <div style="display:flex; align-items:center; gap:0.5rem;">
                                <span style="font-size:1.1rem;">🚚</span>
                                <div>
                                    <div style="font-weight:700; font-size:0.78rem; color:var(--brown-deep,#2C1A0E);">Delivery</div>
                                    <div style="font-size:0.62rem; color:var(--text-muted,#9C8070);">Get your cake delivered</div>
                                </div>
                            </div>
                        </label>
                        <label id="rmf-pickup" style="border:2px solid var(--border,#EAE0D0); border-radius:10px; padding:0.6rem 0.7rem; cursor:pointer; background:white; position:relative; transition:all 0.2s;">
                            <input type="radio" name="rush_match_fulfillment" value="pickup" style="position:absolute;opacity:0;width:0;height:0;">
                            <div style="display:flex; align-items:center; gap:0.5rem;">
                                <span style="font-size:1.1rem;">🏪</span>
                                <div>
                                    <div style="font-weight:700; font-size:0.78rem; color:var(--brown-deep,#2C1A0E);">Pickup</div>
                                    <div style="font-size:0.62rem; color:var(--text-muted,#9C8070);">Collect · Pay cash</div>
                                </div>
                            </div>
                        </label>
                    </div>
                    <div id="rmf-pickup-notice" style="display:none; margin-top:0.5rem; background:#FEF9E8; border:1.5px solid #F0D090; border-radius:8px; padding:0.55rem 0.75rem; font-size:0.72rem; color:#8A5010; line-height:1.5;">
                        🏪 Baker's address shared after confirmation. Pay final 50% in cash when you collect.
                    </div>
                </div>

                <div style="height:1px; background:var(--border,#EDE5DA);"></div>

                {{-- Price breakdown --}}
                @php
                    $rushTotal  = $rushBakerOrder?->agreed_price ?? 0;
                    $rushConfig = is_array($cakeRequest->cake_configuration) ? $cakeRequest->cake_configuration : (json_decode($cakeRequest->cake_configuration, true) ?? []);
                    $cakeBase   = $rushConfig['total'] ?? 0;
                    $rushFeeAmt = $cakeRequest->rush_fee ?? 0;
                    $rushHalf   = round($rushTotal / 2, 2);
                @endphp
                <div>
                    <div style="font-size:0.62rem; text-transform:uppercase; letter-spacing:0.1em; color:var(--text-muted,#9C8070); font-weight:600; margin-bottom:0.5rem;">💰 Price Breakdown</div>
                    <div style="background:var(--cream,#FAF6F1); border:1px solid var(--border,#EDE5DA); border-radius:10px; overflow:hidden;">
                        <div style="display:flex; justify-content:space-between; padding:0.6rem 0.85rem; border-bottom:1px solid var(--border,#EDE5DA);">
                            <span style="font-size:0.73rem; color:var(--text-muted);">Cake Price</span>
                            <span style="font-size:0.85rem; font-weight:700; color:var(--brown-deep);">₱{{ number_format($cakeBase, 2) }}</span>
                        </div>
                        @if($rushFeeAmt > 0)
                        <div style="display:flex; justify-content:space-between; padding:0.6rem 0.85rem; border-bottom:1px solid var(--border,#EDE5DA);">
                            <span style="font-size:0.73rem; color:var(--text-muted);">⚡ Rush Fee</span>
                            <span style="font-size:0.85rem; font-weight:600; color:#C8893A;">+₱{{ number_format($rushFeeAmt, 2) }}</span>
                        </div>
                        @endif
                        <div style="display:flex; justify-content:space-between; padding:0.6rem 0.85rem; border-bottom:1px solid var(--border,#EDE5DA); background:#FEF9E8;">
                            <span style="font-size:0.73rem; font-weight:700; color:var(--brown-deep);">Total</span>
                            <span style="font-size:0.95rem; font-weight:700; color:var(--caramel,#C8894A);">₱{{ number_format($rushTotal, 2) }}</span>
                        </div>
                        <div style="display:flex; justify-content:space-between; padding:0.6rem 0.85rem; border-bottom:1px solid var(--border,#EDE5DA);">
                            <span style="font-size:0.73rem; color:var(--text-muted);">₱ Downpayment (50%)</span>
                            <span style="font-size:0.82rem; font-weight:600; color:#c8862a;">₱{{ number_format($rushHalf, 2) }}</span>
                        </div>
                        <div style="display:flex; justify-content:space-between; padding:0.6rem 0.85rem;">
                            <span id="rmf-final-label" style="font-size:0.73rem; color:var(--text-muted);">🔒 On Delivery (50%)</span>
                            <span style="font-size:0.82rem; font-weight:600; color:var(--text-muted,#9C8070);">₱{{ number_format($rushHalf, 2) }}</span>
                        </div>
                    </div>
                </div>

           <div style="background:#FEF3E8; border:1px solid rgba(200,137,74,0.3); border-radius:8px; padding:0.6rem 0.85rem; font-size:0.72rem; color:#8A4010; line-height:1.5;">
    ⚡ <strong>Your baker has started preparations.</strong> They will request your downpayment shortly — you'll receive a notification when it's time to pay.
</div>
  ...
  {{-- Footer — single button only --}}
<div style="padding:1rem 1.5rem; border-top:1px solid var(--border,#EDE5DA); background:white; flex-shrink:0;">
    <button type="button" onclick="closeRushMatchModal()"
        style="width:100%; padding:0.85rem; border-radius:12px; border:none; background:linear-gradient(135deg,#1A0A00,#7A4A28); color:white; font-size:0.9rem; font-weight:700; cursor:pointer; font-family:'Plus Jakarta Sans', sans-serif; box-shadow:0 4px 14px rgba(59,31,15,0.4); transition:all 0.2s; display:flex; align-items:center; justify-content:center; gap:0.6rem;">
        ✓ Got it — Continue to Order
    </button>
    <p style="text-align:center; font-size:0.72rem; color:var(--text-muted,#9C8070); margin-top:0.6rem; line-height:1.5;">
        Your baker will request payment shortly. You'll be notified when it's time to pay.
    </p>
</div>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
    // Build the map
    @if($bLat && $bLng)
    const rmMap = L.map('rushMatchMap', { zoomControl: false, scrollWheelZoom: false, dragging: true })
        .setView([{{ $bLat }}, {{ $bLng }}], 13);
    L.control.zoom({ position: 'topright' }).addTo(rmMap);
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { attribution:'© OpenStreetMap', maxZoom:19 }).addTo(rmMap);

    const bakerIcon = L.divIcon({ className:'', html:`<div style="width:16px;height:16px;background:#C8894A;border:3px solid white;border-radius:50%;box-shadow:0 2px 8px rgba(200,137,74,0.7);"></div>`, iconSize:[16,16], iconAnchor:[8,8] });
    L.marker([{{ $bLat }}, {{ $bLng }}], { icon: bakerIcon }).addTo(rmMap)
        .bindPopup('🟠 <strong>{{ addslashes($rushBakerUser?->first_name) }}</strong>');

    @if($cLat && $cLng)
    const custIcon = L.divIcon({ className:'', html:`<div style="width:16px;height:16px;background:#3478E8;border:3px solid white;border-radius:50%;box-shadow:0 2px 8px rgba(52,120,232,0.7);"></div>`, iconSize:[16,16], iconAnchor:[8,8] });
    L.marker([{{ $cLat }}, {{ $cLng }}], { icon: custIcon }).addTo(rmMap).bindPopup('🔵 <strong>Your Location</strong>');
    rmMap.fitBounds([[{{ $bLat }},{{ $bLng }}],[{{ $cLat }},{{ $cLng }}]], { padding:[25,25] });
    L.polyline([[{{ $bLat }},{{ $bLng }}],[{{ $cLat }},{{ $cLng }}]], { color:'#C8894A', weight:2, dashArray:'5,5', opacity:0.65 }).addTo(rmMap);
    @endif
    setTimeout(() => rmMap.invalidateSize(), 200);
    @endif

    // Fulfillment toggle
    function setRushMatchFulfillment(mode) {
        const isPickup = mode === 'pickup';
        document.getElementById('rmf-delivery').style.borderColor = isPickup ? 'var(--border,#EDE5DA)' : 'var(--caramel,#C8894A)';
        document.getElementById('rmf-delivery').style.background  = isPickup ? 'white' : '#FEF3E8';
        document.getElementById('rmf-pickup').style.borderColor   = isPickup ? 'var(--caramel,#C8894A)' : 'var(--border,#EDE5DA)';
        document.getElementById('rmf-pickup').style.background    = isPickup ? '#FEF3E8' : 'white';
        document.getElementById('rmf-pickup-notice').style.display = isPickup ? 'block' : 'none';
        document.getElementById('rmf-final-label').textContent     = isPickup ? '💵 Cash on Pickup (50%)' : '🔒 On Delivery (50%)';
        document.querySelectorAll('input[name="rush_match_fulfillment"]').forEach(r => r.checked = r.value === mode);

  // Update tracker hero steps live
    const stepLabels = document.querySelectorAll('.tracker-step-label');
    const stepDots   = document.querySelectorAll('.tracker-step-dot');
    // Rush steps: 0=Submitted,1=Matched,2=Downpay,3=Baking,4=FinalPay,5=Done
    if (stepLabels.length >= 6) {
        stepLabels[4].textContent = isPickup ? 'Pickup'     : 'Final Pay';
        stepLabels[5].textContent = isPickup ? 'Collected'  : 'Delivered';
    }

    // Update hero title/subtitle if currently on WAITING_FINAL_PAYMENT
    const heroTitle = document.querySelector('.hero-title');
    const heroSub   = document.querySelector('.hero-subtitle');
    if (heroTitle && heroSub) {
        const currentStatus = '{{ $cakeRequest->status }}';
        if (currentStatus === 'WAITING_FINAL_PAYMENT') {
            heroTitle.textContent = isPickup
                ? '🏪 Ready for Pickup — Pay Cash'
                : '💰 Pay Final 50%';
            heroSub.textContent = isPickup
                ? 'Your cake is ready! Visit the baker, collect your cake, and pay the remaining balance in cash.'
                : 'Your cake is ready! Pay the remaining balance so your baker can deliver it to you.';
        }
    }
    ...
    if (nextTitle && nextDesc) {
        const currentStatus = '{{ $cakeRequest->status }}';
        if (currentStatus === 'WAITING_FINAL_PAYMENT') {
            nextTitle.textContent = isPickup
                ? 'Action required: Go collect your cake!'
                : 'Action required: Pay final 50%!';
            nextDesc.textContent = isPickup
                ? "Your cake is ready at the baker's location. Visit them, bring the cash, and collect your cake!"
                : 'Your cake is ready! Pay the remaining balance so your baker can deliver it to you.';
        }
    }

    // Update the fulfillment on the order via AJAX
    fetch('{{ route("customer.cake-requests.update-fulfillment", $cakeRequest->id) }}', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}' },
        body: JSON.stringify({ fulfillment_type: mode })
    });
}

    document.getElementById('rmf-delivery').addEventListener('click', () => setRushMatchFulfillment('delivery'));
    document.getElementById('rmf-pickup').addEventListener('click',   () => setRushMatchFulfillment('pickup'));

    // Set initial state from saved fulfillment
    setRushMatchFulfillment('{{ $cakeRequest->fulfillment_type ?? "delivery" }}');
});
function closeRushMatchModal() {
    document.getElementById('rushMatchModal').classList.remove('open');
    document.body.style.overflow = '';
    // Reload so server re-renders the correct pickup/delivery flow
...
if (reviewsList) reviewsList.innerHTML = '';
    ...
    // Parse amount for breakdown
    const numAmount = parseFloat(amount.replace(/[₱,]/g,'')) || 0;
    if (numAmount > 0) {
        const half = (numAmount / 2).toLocaleString('en-PH', {minimumFractionDigits:2, maximumFractionDigits:2});
